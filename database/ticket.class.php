...
class Ticket
{
    ...
    static function updateTicket(PDO $db, int $id, int $agent_id, int $client_id, string $department_id, bool $status)
    {
        $stmt = $db->prepare('UPDATE TICKETS SET agent_id = ?, client_id = ?, department_id = ?, status = ?
    WHERE ticket_id = ?
    ');
        try {
    ...
    {
        $stmt = $db->prepare(
            'SELECT ticket_id,agent_id, client_id, department_id, status,
                 FROM TICKETS
                 LIMIT ?'
        );
    ...
    {
        $stmt = $db->prepare(
            'SELECT ticket_id, agent_id, client_id, department_id, status FROM  TICKETS WHERE ticket_id = ?'
        );
